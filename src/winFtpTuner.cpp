...
void winFtpTuner::drawCircle(int string, int fret, bool pressed)
{
	...
	uint16_t draw_color = pressed ? PRESSED_COLOR : ((fret == 0 || fret > NUM_INTERVALS) ? FRET_COLOR : FRETBOARD_COLOR);
	tft.fillCircle(center_x,center_y,CIRCLE_DIAMETER / 2, draw_color);
...
void winFtpTuner::updateUI()	// draw
{
	...
	if (draw_needed)
	{
		...
		tft.fillRect(		// board
			...
			FRETBOARD_HEIGHT,
			FRETBOARD_COLOR);
		tft.fillRect(		// left bridge
			FRETBOARD_X,
			FRETBOARD_Y,
			BRIDGE_WIDTH,
			FRETBOARD_HEIGHT,
			FRET_COLOR);
		tft.fillRect(		// right bridge
	...
	if (full_draw || t_note != last_tuner_note)
	{
		if (last_tuner_note != -1)
			tft.fillRect(TUNER_NOTE_X,TUNER_NOTE_Y,TUNER_NOTE_WIDTH,TUNER_NOTE_HEIGHT,0);
		if (t_note != -1)
		{
	        tft.setFont(Arial_48);
			tft.setCursor(TUNER_NOTE_X + 5,TUNER_NOTE_Y + 5);
			tft.setTextColor(TFT_WHITE);
            tft.print(noteName(t_note));
		}
