...
void configSystem::updateUI()
{
	...
	if (cur_option != m_last_display_option)
	{
		...
		{
			tft.fillRect(0,TOP_OFFSET,TFT_WIDTH,TFT_HEIGHT-TOP_OFFSET,0);
		...
		if (num >= m_scroll_top && num < m_scroll_top + OPTIONS_PER_PAGE)
		{
			...
			if (draw_all || draw_selected)
			{
				...
				if (color != TFT_BLACK || !draw_all)
					tft.fillRect(0,y,TFT_WIDTH,LINE_HEIGHT-HIGHLIGHT_OFFSET,color);

				uint16_t fc = enabled ? TFT_YELLOW : TFT_DARKGREY;

				tft.setTextColor(fc);
				tft.setCursor(LEFT_OFFSET,y + TEXT_OFFSET);
				tft.print(num+1,DEC);
				tft.print(". ");
				tft.print(opt->getTitle());
			}
