...
void winConfigPedal::updateUI()
{
    ...
    if (full_redraw)
    {
        tft.fillRect(0,Y_OFFSET,480,320-Y_OFFSET,0);
    ...
    if (full_redraw || calib_changed)
    {
       int text_y = Y_OFFSET + 4*RIGHT_LINE_HEIGHT;
        if (!full_redraw)
            tft.fillRect(RIGHT_COL,text_y,RIGHT_WIDTH,2*RIGHT_LINE_HEIGHT,0);
    ...
    {
        int text_y = Y_OFFSET;
        tft.fillRect(RIGHT_COL,text_y,RIGHT_WIDTH,RIGHT_LINE_HEIGHT*2,0);
        if (m_cur_item >= NUM_FIXED_ITEMS)
        {
            ...
            // tft.print(points[cur_point].w,DEC);
        }
    }
        ...
        last_pedal_value != pedal_value)
    {
        ...
        tft.drawLine(X_OFFSET, Y_OFFSET,             X_OFFSET,           Y_OFFSET+CHART_MAX,	TFT_WHITE);
        for (int i=0; i<16; i++)
        {
            int y = Y_OFFSET + i * 16;
            tft.drawLine(X_OFFSET, y, X_OFFSET+9, y, TFT_WHITE);
        }

        // grid center

        tft.drawLine(X_OFFSET, Y_OFFSET+CHART_MAX,   X_OFFSET+CHART_MAX, Y_OFFSET+CHART_MAX, 	TFT_WHITE);  // bottom
        tft.drawLine(X_OFFSET, Y_OFFSET+CHART_MAX,   X_OFFSET+CHART_MAX, Y_OFFSET,				TFT_WHITE);  // diagonal

        // grid right w/ticks

        tft.drawLine(X_OFFSET+CHART_MAX, Y_OFFSET,   X_OFFSET+CHART_MAX,   Y_OFFSET+CHART_MAX, TFT_WHITE);
        for (int i=0; i<16; i++)
        {
            int y = Y_OFFSET + i * 16;
            tft.drawLine(X_OFFSET+CHART_MAX-9, y, X_OFFSET+CHART_MAX, y, TFT_WHITE);
        }
        ...
        pedal_x != last_pedal_x)
    {
        ...
        tft.fillRect(X_OFFSET-1,bar_y,pedal_x*2+3,bar_h,TFT_CYAN);
        tft.fillRect(X_OFFSET+pedal_x*2+3,bar_y,CHART_MAX-pedal_x*2-3,bar_h,0);
